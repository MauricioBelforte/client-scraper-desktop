...
- [ ] **1. Crear Módulo de Constantes:** Crea la carpeta `src` y dentro el archivo `src/constants.py`.
- [ ] **2. Mover `RUBROS_SUGERIDOS`:** Corta la lista `rubros_sugeridos` de `lead_app.py` y pégala en `src/constants.py` como `RUBROS_SUGERIDOS`. Importa y úsala en `setup_ui`.
