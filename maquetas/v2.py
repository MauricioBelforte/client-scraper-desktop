...
def generar_maqueta_v2(datos):
    ...
    if direccion:
        ...
        <section id="ubicacion" class="seccion-presentacion">
    ...
    <footer id="contacto" style="background: #181818; padding: 4rem 2rem; text-align: center; color: #aaa; font-size: 0.9rem;">
        ...
        <div class="social-icons">
            {'<a href="' + instagram_url + '" target="_blank" class="social-icon instagram active-link" aria-label="Instagram">' + instagram_svg + '</a>' if has_instagram else ''}
            {'<a href="' + facebook_url + '" target="_blank" class="social-icon facebook active-link" aria-label="Facebook">' + facebook_svg + '</a>' if has_facebook else ''}
        </div>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    main {{ padding-top: 85px; }} /* Espacio para la barra de navegación fija */
    .seccion-hero {{ 
        text-align: center; 
        padding: 4rem 2rem; 
        min-height: 80vh; 
    ...
    .social-icon svg {{ width: 40px; height: 40px; color: #aaa; transition: all 0.3s; }}
    .social-icon:hover svg {{ color: var(--color-primario); transform: scale(1.1); }}
